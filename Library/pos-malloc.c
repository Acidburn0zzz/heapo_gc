...
char* Alloc_tree = "Alloc_tree";
int alloc_tree_counter = 0;
int alloc_tree_meta_update_flag = 0;
int alloc_tree_init_flag = 1;
unsigned int input_key = 0;
unsigned int input_value = 0;
int value_test;
extern struct pos_name_entry *name_table[];
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
void insert_lock(){ pthread_mutex_lock(&mutex);}
void insert_unlock(){ pthread_mutex_unlock(&mutex);}
//dk e
...
/*
static void garbage_marking(char *name, unsigned int oid)
{
	void *ptr;

	int i, height;
	unsigned long *node = ptr->node;
	...
	for ( ; height > 1; height--)
	{
		...
		{
			printf("address = %p, value = %lu\n", &(bval(geo, node, i)), bval(geo, node, i);
		}
...
errout:
			...
			*fb = victim->fd;
			...
			if(alloc_tree_init_flag == 1 && alloc_tree_meta_update_flag == 0)
			{
				...
				printf("oid = %d\n", name_table[index]->oid);
				value_test = name_entry->oid;
				*/
				...
				if(POS_DEBUG == 1)
				{
					...
					printf("ptr = %p\n", ptr);
					//pos_btree_traverse(Alloc_tree);
					root_node_ptr=pos_get_prime_object(Alloc_tree);
					node_ptr = root_node_ptr->node;
					printf("===== pos_btree_traverse start ===== \n");
					pos_btree_traverse2(node_ptr);
					printf("===== pos_btree_traverse end ===== \n");
				}
			}
...
// Remainder dosen't need logging...
		...
		for (;;) {
			...
			if (victim == bin) {
				...
				//dk s
				//memcpy(&input_key, &p, 8);
				//memcpy(&input_value, &value_test, 8);
				if(POS_DEBUG ==1)
				{
					//printf("input_key = %x\n", input_key);
					//printf("input_value = %x\n", input_value);
				}
				if(alloc_tree_init_flag == 1  &&  alloc_tree_meta_update_flag == 0)
				{
					int return_val;
				...
				*/
					...
					if(POS_DEBUG == 1)
					{
						...
						pos_btree_traverse2(node_ptr);
						printf("===== pos_btree_traverse end ===== \n");
						//garbage_collection_list(name);
						/*
						root_node_ptr=pos_get_prime_object(name);
						printf("root_node_ptr = %p\n", root_node_ptr);
						node_ptr = root_node_ptr->node;
						printf("node_ptr = %p\n", root_node_ptr->node);
						pos_btree_gc(node_ptr);
						*/
					}
				}
				//dk e
...
//#endif
			...
			if(alloc_tree_init_flag == 1 && alloc_tree_meta_update_flag == 0)
			{
				...
				if(POS_DEBUG == 1)
				{
					ptr=pos_btree_lookup(Alloc_tree, &p2);
					printf("ptr = %p\n", ptr);
					//pos_btree_traverse(Alloc_tree);
					root_node_ptr=pos_get_prime_object(Alloc_tree);
					node_ptr = root_node_ptr->node;
...
errout:
	...
	if(alloc_tree_init_flag == 1  &&  alloc_tree_meta_update_flag == 0)
	{
		if(POS_DEBUG == 1)
		{
			printf("pos_free fastbin\n");
		}

		if(POS_DEBUG == 1)
		{
			ptr=pos_btree_lookup(Alloc_tree, &ptr2);
			printf("before free ptr = %p\n", ptr);
		}
		pos_btree_remove(Alloc_tree, &ptr2);
...
/*#if CONSISTENCY == 1
	...
	if(POS_DEBUG == 1)
	{
		printf("------ pos_set_prime_object-----\n");
		printf("obj = %p\n", obj);
		printf("ptr->prime obj = %p\n", ar_ptr->prime_obj);
		printf("------------------------------\n");
	}
}
